...
export async function generateContentConcept(): Promise<any> {
  ...
  - 최근 30회 타겟 성별 분포: 여성 ${femaleCount}회, 남성 ${maleCount}회. (목표 비율: 여성 70%, 남성 30%)
  - 최근 발행된 기획안 주제들:
    ${recentConcepts.length > 0 ? recentConcepts.map((c, i) => `${i+1}. ${c}`).join('\n    ') : '이력이 없습니다.'}
  
  ⚠️ 중요 지침: 매일 인스타 피드가 지루해지지 않고 최적화되도록, 최근 한 달간 발행된 '주제' 및 '포맷(예: 시술 전후 비교, 정보성 카드뉴스, Q&A, 고객 후기 등)'이 연속해서 겹치지 않도록 완전히 새로운 포맷으로 로테이션하여 기획하세요.`;
