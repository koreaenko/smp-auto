...
- 광고 느낌보다 "고객 고민 공감형"
- 짧은 3~4줄짜리 캡션 금지. 반드시 충분한 분량(최소 15줄 이상)으로 깊이 있게 작성할 것.
...
2단계 - 후킹 + 공감 (2~3줄):
고객의 고민을 짧은 문장, 줄바꿈으로 공감해줌.

3단계 - 해결 방향 설명 (3~4줄):
"부담스럽지 않게, 티 나지 않게" 철학을 녹여서, 어떤 방식으로 보완하는지 자연스럽게 설명.

4단계 - 전문성/디테일 강조 (2~3줄):
왜 이 부위가 중요한지, 더 톡이 어떤 점을 중요하게 생각하는지 전문적으로 설명.

5단계 - 서비스 체크리스트 (✔ 사용):
✔ 항목 3~5개로 핵심 서비스를 정리.

6단계 - 철학 마무리 + CTA:
"티 안 나게 자연스럽게" 같은 브랜드 철학 한 줄 + 📍안양 / 평촌 / 범계 + 📩 상담 및 예약 DM

🚨 [캡션 퀄리티 기준 - 반드시 이 수준으로 작성하십시오]
아래는 실제 운영 중인 캡션 예시입니다. 이 분량, 이 톤, 이 구조를 반드시 따라야 합니다:
...
과하게 채우는 방식이 아니라
기존 모발 흐름에 자연스럽게 어우러질 수 있도록
밀도감을 보완해드립니다.

정수리는 작은 차이만으로도
전체 인상이 달라 보일 수 있는 부위인 만큼
자연스러운 균형과 밀도를 중요하게 생각합니다.
...
"티 안 나게 자연스럽게"
더 톡이 가장 중요하게 생각하는 방향입니다.

📍안양 / 평촌 / 범계
📩 상담 및 예약 DM
---예시 끝---

위 예시처럼 최소 15줄 이상, 줄바꿈 많이, ✔ 체크리스트 포함, 철학 마무리 문구 포함으로 작성하십시오.
3~5줄짜리 짧은 캡션은 절대 금지입니다.

[캡션 규칙]
- 줄바꿈 많게, 모바일 가독성 우선
- 한 문장은 짧게, 하지만 전체 분량은 충분히 풍성하게
- "자연스럽게", "부담스럽지 않게", "티 나지 않게" 반복 활용
- 마지막에 반드시 "📍안양 / 평촌 / 범계" + "📩 상담 및 예약 DM" 포함

[해시태그 생성 규칙]
🚨 [필수 강제 포함]: 아래 해시태그는 무조건, 하나도 빠짐없이 포함되어야 합니다:
"#더톡 #안양두피문신 #안양SMP #평촌두피문신 #범계두피문신"

추가로 아래에서 콘텐츠에 맞는 태그를 골라 총 20~25개로 작성합니다:
- 서비스: #두피문신 #SMP #두피타투 #정수리커버 #정수리두피문신 #헤어라인교정 #밀도보강 #M자교정 #탈모커버 #탈모관리 #가르마커버 #가르마비움
- 여성/타겟: #여성두피문신 #여성헤어라인 #잔머리교정
- 지역 뷰티: #안양뷰티 #평촌뷰티 #범계뷰티

반드시 아래 JSON 형식으로만 완벽하게 응답해야 하며, 다른 텍스트는 포함하지 마십시오.
JSON Response Schema:
{
  "caption": "줄바꿈(\\n) 및 이모지, ✔ 체크리스트 포함, 최소 15줄 이상의 완성형 캡션 텍스트",
  "hashtags": "#더톡 #안양두피문신 #안양SMP #평촌두피문신 #범계두피문신 + 추가 태그 (공백 구분, 총 20~25개)"
}
`;
